class GradientRamp
  {
          ...
          if(this.stops.length == 0)
            {
              ...
              this.stops = [new GradientStop(0, color('black')), new GradientStop(1, color('white'))];
            }
          else
            {
    ...
    sampleColor(samplePosition, withNoise, noiseAmount, noiseArea, withOffsets)
    {
      ...
      let interpolationValue = (nextStop.position - samplePosition) / (nextStop.position - previousStop.position);
      ...
      let sampledColor = lerpColor(nextStop.stopColor, previousStop.stopColor, interpolationValue);
    ...
    getColorSamples(count, withNoise, noiseAmount, noiseArea, withOffsets)
    {
      ...
      for(let c = 0; c < count; c++)
        {
          let gradientColor = this.sampleColor(c/count, withNoise, noiseAmount, noiseArea, withOffsets);
