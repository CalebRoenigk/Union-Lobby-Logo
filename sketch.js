...
function Logo_Block()
{
  class BlockRenderer
  {
    constructor()
    {
      ...
      this.dashColors = [color('#fba809'),color('#8a7328'),color('#0b4727'),color('#fc93bb'),color('#fd2f0b'),color('#090909'),color('#a6b4b4'),color('#0657d0'),color('#ecf9f8')];
    ...
    generateRowDashes()
    {
      ...
      for(let r = 0; r < this.rows; r++)
        {
          let totalDashes = round(random(this.rowDashMinMax.x, this.rowDashMinMax.y));
          let dashSet = [];
          let dashGradientSet = [];
          let dashColorSet = [];
          ...
          for(let d = 0; d < totalDashes; d++)
            {
              let dashLength = random(this.dashLengthMinMax.x, this.dashLengthMinMax.y);
              let dashGradient = 1 - random(0, 1) > 0.75;
              let dashColor = this.dashColors[round(random(0, this.dashColors.length - 1))];
              
              let gradientStops = round(random(this.rowGradientStopsMinMax.x, this.rowGradientStopsMinMax.y));
              let dashGradientColorStops = [];
              let remainingValue = 1;
              for(let s = 0; s < gradientStops; s++)
                {
                  let stopPosition = Clamp((round((Clamp(randomGaussian(0.5), 0.25, 1) * (remainingValue/(gradientStops - s)))*10)/10) * 1.25, 0, 1);
                  let stop = new GradientStop(stopPosition, this.dashColors[round(random(0, this.dashColors.length - 1))]);
              ...
              if(dashGradientColorStops[dashGradientColorStops.length - 1].position < 0.9)
                {
                  let maxStopPosition = dashGradientColorStops[dashGradientColorStops.length - 1].position;
                  for(let s = 0; s < dashGradientColorStops.length; s++)
                    {
                      if(dashGradientColorStops[s].position != 0)
                        {
                          dashGradientColorStops[s].position = round(map(dashGradientColorStops[s].position, 0, maxStopPosition, 0, 1, true)* 10)/10;
                        }
                    }
                }
    ...
    updateRowThickness()
    {
      ...
      for(let r = 0; r < this.rows - 1; r++)
        {
          let rowAddition = (height / (this.rows - 1)) * r + ((height / (this.rows - 1)) / 2);
          if(r % 2 == 0)
            {
              let rowBase = (height / (this.rows - 1)) * 0.25;
              rowAddition += SineWave(rowBase, this.rowFrequency[r] / 3, r, rowBase/2, 0.25, this.time);
            }
          else
            {
              let rowBase = (height / (this.rows - 1)) * 0.125;
              rowAddition += SineWave(rowBase, this.rowFrequency[r] / 2, r, rowBase/2, 0.35, this.time);
            }
          ...
          let rowPosition = p5.Vector.lerp(createVector(0, weightTotal), createVector(0, weightTotal + weight), 0.5);
      ...
      this.rowWeight.push(createVector(0, height).dist(this.rowPosition[this.rowPosition.length - 1]));
      this.rowPosition.push(p5.Vector.lerp(this.rowPosition[this.rowPosition.length - 1], createVector(0, height), 0.75));
    }
      ...
      for(let a = 0; a < this.rowDashes[r].length;)
        {
          ...
          let xOffset = SineWave(dashLength * 2, 0.375 + ((r - 4) * 0.02), r, 0, 1, this.time) * direction * dashOffset;
          
          drawingContext.lineDashOffset = xOffset;
          
          if(gradientFill)
            {
              let gradient = drawingContext.createLinearGradient((0+ dashTotal) - xOffset, height, (0 + dashTotal + dash) - xOffset, height);
        
              for(let s = 0; s < this.rowGradientColors[r][a].stops.length; s++)
                {
                  gradient.addColorStop(this.rowGradientColors[r][a].stops[s].position, this.rowGradientColors[r][a].stops[s].stopColor);
                }
  ...
  class Stripe
    {
      ...
      createBasePoints()
      {
        ...
        for(let p = 0; p < this.rows; p++)
          {
            let centerPoint = createVector(center.x, (rowHeight * p + (rowHeight/2)) - (this.height - height)/2);
            ...
            for(let p = 0; p < this.rows; p++)
              {
                ...
                let x = SineWave(thisWave.amplitude, thisWave.frequency, thisWave.phase + p, thisWave.verticalOffset, thisWave.speed, this.time);
                this.displacedPoints[p] = createVector(this.displacedPoints[p].x + x, this.displacedPoints[p].y);
              }
          }
      }
        ...
        for(let d = 0; d < this.rows * 2; d++)
          {
            ...
            let pointA = createVector(centerPoint.x + ((this.width/2) * xSide), centerPoint.y + ((rowHeight/2) * xSide));
            let pointB = createVector(centerPoint.x + ((this.width/2) * xSide), centerPoint.y - ((rowHeight/2) * xSide));
      ...
      fillGradient()
      {
        ...
        let gradientShuffle = SineWave(this.gradientSineWave.amplitude, this.gradientSineWave.frequency, this.gradientSineWave.phase, this.gradientSineWave.verticalOffset, this.gradientSineWave.speed, this.time);
        let gradient = drawingContext.createLinearGradient(0, (-height*2) + gradientShuffle, width, (height * 3) + gradientShuffle);
        ...
        for(let g = 0; g < this.gradientStops; g++)
          {
            ...
            this.gradient.push({step: Clamp(gradientStopValue + thisStep, 0, 1), color: randomGrayscale});
    ...
    for(let s = 0; s < 5; s++)
      {
        ...
        let sineWave1 = new SineWaveProperties(random(wave1AmpRange.x, wave1AmpRange.y), random(wave1FreqRange.x, wave1FreqRange.y), random(wavePhaseRange.x, wavePhaseRange.y), 0, random(wave1SpeedRange.x, wave1SpeedRange.y));
        let sineWave2 = new SineWaveProperties(random(wave2AmpRange.x, wave2AmpRange.y), random(wave2FreqRange.x, wave2FreqRange.y), random(wavePhaseRange.x, wavePhaseRange.y), 0, random(wave2SpeedRange.x, wave2SpeedRange.y));
        let sineWave3 = new SineWaveProperties(random(wave3AmpRange.x, wave3AmpRange.y), random(wave3FreqRange.x, wave3FreqRange.y), random(wavePhaseRange.x, wavePhaseRange.y), 0, random(wave3SpeedRange.x, wave3SpeedRange.y));
        
        let gradientSineWave = new SineWaveProperties(random(gradientAmpRange.x, gradientAmpRange.y), random(gradientFreqRange.x, gradientFreqRange.y), random(wavePhaseRange.x, wavePhaseRange.y), 0, random(gradientSpeedRange.x, gradientSpeedRange.y));
        
        this.stripes.push(new Stripe(randomX, [sineWave1, sineWave2, sineWave3], gradientSineWave));
      }
        ...
        for(let x = 0; x < this.resolution; x++)
          {
            for(let y = 0; y < this.resolution; y++)
              {
                let position = createVector(x * (width / this.resolution), y * (height / this.resolution));
                let size = createVector(width / this.resolution, height / this.resolution);
                let noiseTime = time * this.noiseSpeed;
                let noiseSample = noise((x * this.noiseSize) + noiseTime, (y * this.noiseSize) + noiseTime,noiseTime);
      ...
      renderSquare(position, size, squareColor, resolution)
        {
          ...
          let start = this.rotatePoint(createVector(0,0), lineStart, hueRotation, false);
          let end = this.rotatePoint(createVector(0,0), lineEnd, hueRotation, false);
          ...
          for(let x = 0; x < resolution; x++)
            {
              for(let y = 0; y < resolution; y++)
                {
                  let centerPoint = createVector(position.x + (x * (size.x/resolution)) + ((size.x/resolution)/2), position.y + (y * (size.y/resolution)) + ((size.y/resolution)/2));
...
function Logo_Strings()
{
  class StringRenderer
  {
    constructor()
    {
      ...
      this.stringSampleGradient = new GradientRamp([new GradientStop(0, color('#1b87ff')), new GradientStop(0.4, color('#4f119e')), new GradientStop(0.75, color('#ff6f00')), new GradientStop(1, color('#ffc000'))]);
      ...
      this.minSineDisplacement = new SineWaveProperties(this.minDisplacementAmplitude, this.minDisplacementFrequency,               this.minDisplacementPhase, this.minDisplacementVerticalOffset, this.minDisplacementSpeed);
      ...
      this.maxSineDisplacement = new SineWaveProperties(this.maxDisplacementAmplitude, this.maxDisplacementFrequency,               this.maxDisplacementPhase, this.maxDisplacementVerticalOffset, this.maxDisplacementSpeed);
    ...
    createStrings()
    {
      ...
      for(let s = 0; s < this.stringCount; s++)
        {
          let stringColor = this.stringSampleGradient.sampleColor(s / this.stringCount);
          
          let anchorRange = createVector(this.anchorMargin * width, (this.anchorMargin * width) + (this.anchorMargin * width * this.anchorMarginWidth));
          ...
          let anchor1 = createVector(-random(anchorRange.x, anchorRange.y), ((s / this.stringCount) * height));
          let control1 = createVector(anchor1.x - random(anchorRange.x, anchorRange.y), random(0 - (height * this.heightMargin), height + (height * this.heightMargin)));
          ...
          let anchor2 = createVector(width + random(anchorRange.x, anchorRange.y), ((s / this.stringCount) * height));
          let control2 = createVector(anchor2.x + random(anchorRange.x, anchorRange.y), random(0 - (height * this.heightMargin), height + (height * this.heightMargin)));
          ...
          let lineString = new LineString(this, anchor1, control1, anchor2, control2, anchor1SineProperties, control1SineProperties, anchor2SineProperties, control2SineProperties, stringColor, s);
    ...
    getSineProperties()
    {
      let amplitude = random(this.minSineDisplacement.amplitude, this.maxSineDisplacement.amplitude);
      let frequency = random(this.minSineDisplacement.frequency, this.maxSineDisplacement.frequency);
      let phase = random(this.minSineDisplacement.phase, this.maxSineDisplacement.phase);
      let verticalOffset = random(this.minSineDisplacement.verticalOffset, this.maxSineDisplacement.verticalOffset);
      let speed = random(this.minSineDisplacement.speed, this.maxSineDisplacement.speed);
      
      return new SineWaveProperties(amplitude, frequency, phase, verticalOffset, speed);
    }
  }
  ...
  class LineString
  {
    ...
    renderString()
    {
          ...
          for(let h = 0; h < this.handlePoints.length; h++)
            {
              ...
              text(stringName + textValue + pointNumber.toString(), this.handlePoints[h].x, this.handlePoints[h].y);
            }
        }
      ...
      bezier(handlePoints[1].x, handlePoints[1].y, handlePoints[0].x, handlePoints[0].y, handlePoints[2].x, handlePoints[2].y, handlePoints[3].x, handlePoints[3].y);
    }
    
    displaceHandles()
    {
      // Displace the handles from their base positions
      let anchor1 = this.getHandlePointOffset(this.anchor1, this.anchor1SineProperties.amplitude, this.anchor1SineProperties.frequency, this.anchor1SineProperties.phase, this.anchor1SineProperties.verticalOffset, this.anchor1SineProperties.speed, 1.35);
      let control1 = this.getHandlePointOffset(this.control1, this.control1SineProperties.amplitude, this.control1SineProperties.frequency, this.control1SineProperties.phase, this.control1SineProperties.verticalOffset, this.control1SineProperties.speed, -2.81);
      let anchor2 = this.getHandlePointOffset(this.anchor2, this.anchor2SineProperties.amplitude, this.anchor2SineProperties.frequency, this.anchor2SineProperties.phase, this.anchor2SineProperties.verticalOffset, this.anchor2SineProperties.speed, -0.56);
      let control2 = this.getHandlePointOffset(this.control2, this.control2SineProperties.amplitude, this.control2SineProperties.frequency, this.control2SineProperties.phase, this.control2SineProperties.verticalOffset, this.control2SineProperties.speed, 3.3);
      
      return [anchor1, control1, anchor2, control2];
    }
    
    getHandlePointOffset(point, amplitude, frequency, phase, verticalOffset, speed, yPhaseOffset)
    {
      let xOffset = SineWave(0, frequency, phase, verticalOffset, speed, this.time);
      let yOffset = SineWave(amplitude, frequency, phase + yPhaseOffset, verticalOffset, speed, this.time);
...
function Logo_Radiant()
{
  class PointRenderer
  {
    constructor(count, center)
    {
      ...
      this.gradient = new GradientRamp([new GradientStop(0, color('#1b87ff')), new GradientStop(0.4, color('#4f119e')), new GradientStop(0.75, color('#ff6f00')), new GradientStop(1, color('#ffc000'))]);
    ...
    renderPoints(time)
    {
      ...
      this.colorTransitionTime -= deltaTime / (1000 * this.colorTransitionDuration);
      ...
      for(let p = 0; p < this.points.length; p++)
        {
          let pointColor = lerpColor(this.colors[p], this.nextColors[p], 1 - this.colorTransitionTime / 1);
          
          noFill();
          stroke(pointColor);
          let pointSize = this.pointSize + SineWave((p / this.points.length) * this.pointSize * 1.25, 0.375, this.pointSizePhases[p], 0.25, 4, this.time);
          strokeWeight(pointSize);
          
          // Only render points in the viewport, includes the margins of the viewport
          if((this.points[p].x >= -this.viewportRenderMargin && this.points[p].x <= width + this.viewportRenderMargin) && (this.points[p].y >= -this.viewportRenderMargin && this.points[p].y <= height + this.viewportRenderMargin))
            {
    ...
    rotatePoint(index)
    {
      ...
      let rotatedX = Math.cos(angle) * (point.x - this.center.x) - Math.sin(angle) * (point.y - this.center.y) + this.center.x;
      let rotatedY = Math.sin(angle) * (point.x - this.center.x) + Math.cos(angle) * (point.y - this.center.y) + this.center.y;
        ...
        for(let x = 0; x < this.resolution; x++)
          {
            for(let y = 0; y < this.resolution; y++)
              {
                let noiseSample = noise((x + (time * this.noiseSpeed.x)) * this.noiseSize.x, (y + (time * this.noiseSpeed.y)) * this.noiseSize.y, ((x *y) + (time * this.noiseSpeed.z)) * this.noiseSize.z);
                
                let center = createVector(x * (width / this.resolution), y * (height / this.resolution)).add(createVector((width / this.resolution)/2, (height / this.resolution)/2));
                let size = createVector(this.scaleSize.x * noiseSample, this.scaleSize.y * noiseSample);
                ...
                let sine = SineWave(12, 0.25 + (noiseSample/100), ((x * y) + (x + y)) + noiseSample, 0, 7, this.time);
      ...
      for(let x = 0; x < this.stationResolution + 2; x++)
        {
          ...
          for(let y = 0; y < this.stationResolution + 2; y++)
            {
              let xPos = ((x * (width / this.stationResolution) + (((width / this.stationResolution)) / 2)) - ((width / this.stationResolution)));
              let yPos = ((y * (height / this.stationResolution) + (((height / this.stationResolution)) / 2)) - ((height / this.stationResolution)));
              ...
              stationColorRow.push(color(random(this.stationColorRange.x,this.stationColorRange.y)));
            }
      ...
      for(let t = 0; t < this.trainCount; t++)
        {
          ...
          let trainSpeed = random(this.trainSpeedMinMax.x, this.trainSpeedMinMax.y);
          let train = new Train(this, line, trainCarDashes, color(random(this.trainColorRange.x,this.trainColorRange.y)), trainSpeed);
      ...
      for(let d = 0; d < trainIterations; d++)
        {
          ...
          let trainLength = floor(random(this.trainLengthMinMax.x,this.trainLengthMinMax.y)) * stationGapWidth;
          let gapLength = floor(random(this.trainGapLengthMinMax.x,this.trainGapLengthMinMax.y)) * stationGapWidth;
...
class Train
  {
      ...
      for(let s = 0; s < this.manager.stationResolution + 2; s++)
        {
          ...
          if(!this.canGoOppisiteDirection && direction == this.cooldownDirection)
            {
              if(direction == -1)
                {
                  direction = 1;
              ...
              if(currentStation.y == 0)
                {
                  ...
                  stop = this.manager.stations[currentStation.x - 1][currentStation.y + 1];
                  currentStation.y += 1;
                }
              else
                {
    ...
    getRailLength()
    {
      ...
      for(let p = 0; p < this.stops.length - 1; p++)
        {
          let nextStopDistance = dist(this.stops[p].x, this.stops[p].y, this.stops[p + 1].x, this.stops[p + 1].y);
    ...
    renderTrain()
    {
      ...
      drawingContext.lineDashOffset = (this.manager.time * this.speed) + this.offset;
...
function Logo_Link()
{
  class Link
    {
      constructor(manager, start, size, linkColor, handleColor, linkIndex, travelInterval)
      {
        this.manager = manager;
        ...
        if(this.leadingLines)
          {
            ...
            strokeWeight(1);
            line(this.targetAVisual.x, this.targetAVisual.y, this.pointA.x, this.pointA.y);
          }
      }
        ...
        if(this.debug)
          {
            ...
            strokeWeight(1);
            line(this.pointB.x, this.pointB.y, this.targetPointB.x, this.targetPointB.y);

            noStroke();
            fill(color('blue'));
            text("Travel: " + (round((this.travelBTime / this.travelDuration)*10)/10), this.pointB.x, this.pointB.y + 10);
            ...
            strokeWeight(1);
            line(this.pointA.x, this.pointA.y, this.targetPointA.x, this.targetPointA.y);

            noStroke();
            fill(color('red'));
            text("Travel: " + (round((this.travelATime / this.travelDuration)*10)/10), this.pointA.x, this.pointA.y);
          }
      }
        ...
        if(this.isATraveling)
          {
            ...
            if(this.travelATime >= this.travelDuration)
            ...
            else
              {
                ...
                let x = lerp(this.pointA.x, this.targetPointA.x, easeInOutSine(aTravel));
                let y = lerp(this.pointA.y, this.targetPointA.y, easeInOutSine(aTravel));
            ...
            if(this.leadingLines)
              {
                ...
                let x = lerp(this.targetAVisual.x, this.targetPointA.x, easeInOutSine(aBreak));
                let y = lerp(this.targetAVisual.y, this.targetPointA.y, easeInOutSine(aBreak));
        ...
        if(this.isBTraveling)
          {
            ...
            if(this.travelBTime >= this.travelDuration)
            ...
            else
              {
                ...
                let x = lerp(this.pointB.x, this.targetPointB.x, easeInOutSine(bTravel));
                let y = lerp(this.pointB.y, this.targetPointB.y, easeInOutSine(bTravel));
        ...
        if(point == 'A')
          {
            ...
            if(this.aIndex.x <= 0 || this.aIndex.x >= this.manager.stations.length - 1 || this.aIndex.y <= 0 || this.aIndex.y >= this.manager.stations[0].length - 1)
              {
            ...
            else
              {
                ...
                this.targetPointB = p5.Vector.add(this.targetPointA, createVector(0,0));
                this.targetPointA = createVector(this.manager.stations[this.aIndex.x][this.aIndex.y].x,this.manager.stations[this.aIndex.x][this.aIndex.y].y);
  ...
  class StationPopulationChangeIndicator
    {
      constructor(manager, position, delay, duration, indicatorFillOpacityModifier, indicatorIndex)
      {
        this.manager = manager;
        ...
        if(this.delay <= 0)
          {
            ...
            let animationInterpolation = easeOutCubic(this.playTime / this.duration);
            if(this.direction == 1)
              {
                animationInterpolation = easeInCubic(this.playTime / this.duration);
              }

            let indicatorOpacity = round(lerp(this.startOpacity, this.endOpacity, animationInterpolation)*100)/100;
            let indicatorSize = round(lerp(this.startSize, this.endSize, animationInterpolation)*100)/100;

            // Render the indicator at the correct display time
            ellipseMode(CENTER);
            stroke(color('rgba(44,66,81,' + indicatorOpacity + ')'));
            strokeWeight(1);
            fill('rgba(183,206,206,' + (round((indicatorOpacity * this.indicatorFillOpacityModifier)*10)/10) + ')');
  ...
  class Arrival extends StationPopulationChangeIndicator
    {
      constructor(manager, position, delay, duration, indicatorSize, indicatorFillOpacityModifier, indicatorIndex)
      {
        super(manager, position, delay, duration, indicatorFillOpacityModifier, indicatorIndex);
        this.direction = 0;
        this.startSize = 0;
  ...
  class LinkManager
  {
    constructor()
    {
      ...
      this.linkColors = [color('#2C4251'), color('#FCA311'), color('#B7CECE'), color('#2E80C7')];
      ...
      for(let x = 0; x < this.stationResolution + 6; x++)
        {
          ...
          for(let y = 0; y < this.stationResolution + 6; y++)
            {
              let xPos = ((x * (width / this.stationResolution) + (((width / this.stationResolution)) / 2)) - ((width / this.stationResolution) * 3));
              let yPos = ((y * (height / this.stationResolution) + (((height / this.stationResolution)) / 2)) - ((height / this.stationResolution) * 3));
              let station = createVector(xPos, yPos);
              stationRow.push(station);
    ...
    generateLink()
    {
      let randX = floor(Clamp(randomGaussian(((this.stationResolution + 6) - 4) / 2, 5), 1, this.stationResolution - 2)) + 3;
      let randY = floor(Clamp(randomGaussian(((this.stationResolution + 6) - 4) / 2, 5), 1, this.stationResolution - 2)) + 3;
      let linkSpawn = createVector(randX, randY);
      let linkColor = this.linkColors[round(random(0, this.linkColors.length-1))];
      let handleColor = this.getHandleColor(linkColor);
      let travelInterval = round(random(1, 4));
      let newLink = new Link(this, linkSpawn, createVector(width / this.stationResolution, this.linkSize), linkColor, handleColor, this.links.length, travelInterval);
          ...
          for(let y = 0; y < this.stationPopulation[0].length; y++)
            {
              ...
              populationRowChanges.push((thisStationPopulation - lastFrameStationPopulation));
            }
                  ...
                  if(abs(populationChanges[x][y]) > 2)
                    {
                      ...
                      if(populationChanges[x][y] < 0)
                        ...
                        {
                          // Arrivals
                          this.populationChangeRender.push(new Arrival(this, indicatorPosition, indicatorDelay, this.populationArrivalIndicatorDuration, this.indicatorSize, this.indicatorFillOpacityModifier, indicatorIndex));
                        }
                    }
                }
            }
    ...
    getHandleColor(linkColor)
    {
      ...
      let newBrightness = round(Clamp(brightness(linkColor) - brightnessShift, 0, 100));
      return color('hsb(' + newHue + ', ' + newSaturation + '%, ' + newBrightness + '%)');
    }
...
function Logo_Holographic()
{
  class GradientManager
    {
        ...
        for(let x = 0; x < this.divisions; x++)
          {
            ...
            let sectionOffset = createVector(((x + 1) - (this.divisions / 2)) * (width/this.divisions), this.sectionOffsets[x]);
            ...
            for(let b = 0; b < this.bubbles.length; b++)
              {
                ...
                noStroke();
                fill(color('red'));
                this.radialGradient(bubble.position, bubble.radius, bubble.color, sectionOffset);
        ...
        for(let b = 0; b < this.bubbleCount; b++)
          {
            ...
            let bubblePosition = createVector(random(0 + (width / 10), width - (width / 10)), random(0 + (height / 10), height - (height / 10)));
            let bubbleColor = this.bubbleColorsSample[floor(random(0, this.bubbleColorsSample.length - 1))];
            this.bubbles.push(new GradientBubble(this, bubblePosition, bubbleRadius, bubbleColor));
          }
      }
      ...
      generateGradient()
      {
        ...
        for(let y = 0; y < this.gradientColors + 1; y++)
          {
            let stopColor = this.bubbleColorsSample[floor(random(0, this.bubbleColorsSample.length - 1))];
      ...
      updateSectionOffsets()
      {
        for(let x = 0; x < this.divisions; x++)
          {
            this.sectionOffsets[x] = round(SineWave(height * this.offsetAmplitude, 0.25, x * (x + x) * ((x + x) * (x + x)) , 0, 2, this.time));
          }
      }
      ...
      linearGradient(offset)
      {
          let gradient = drawingContext.createLinearGradient(
            0 + offset.x, (height + (height * this.offsetAmplitude)) + offset.y, width  + offset.x, (0 - (height * this.offsetAmplitude)) + offset.y
          );
          
          for(let s = 0; s < this.gradient.stops.length; s++)
            {
              gradient.addColorStop(this.gradient.stops[s].position, this.gradient.stops[s].stopColor);
            }
      ...
      radialGradient(center, radius, fillColor, offset)
      {
        let gradient = drawingContext.createRadialGradient(
          center.x, center.y + offset.y, 0, center.x, center.y + offset.y, radius
        );

        gradient.addColorStop(0, fillColor);
        gradient.addColorStop(1, color('rgba(' + [red(fillColor), green(fillColor), blue(fillColor)].join(',') + ', 0)'));
  ...
  class GradientBubble
    {
      ...
      updateBubble()
      {
        if(this.position.x + this.velocity.x < 0 || this.position.x + this.velocity.x > width)
          {
            this.velocity.x *= -1;
          }
        if(this.position.y + this.velocity.y < 0 || this.position.y + this.velocity.y > height)
          {
            this.velocity.y *= -1;
          }
  ...
  this.bubbleColorsSample = [color('#465b9a'), color('#e64685'), color('#7300a6'), color('#2c48fe'), color('#38b5b4'), color('#a6fe28'), color('#009e75'), color('#d93958'), color('#fea8ef')];
  ...
  class RaceRenderer
    {
        ...
        for(let c = 0; c < this.count; c++)
          {
            ...
            circle(this.center.x, this.center.y, (this.circleMaxSize / this.count) * (c + 1))
          }
        ...
        for(let p = 0; p < this.count; p++)
          {
            ...
            let t2 = this.slantedSineWave(0.5, 4, p/this.count, (p/this.count) * 3, 0.5, 0.25);
            
            let pointOnCircle = this.getPointOnCircle(t, ((this.circleMaxSize / this.count) * (p + 1)) / 2);
            pointOnCircle.add(this.center);
            let pointOnCircle2 = this.getPointOnCircle(t2, ((this.circleMaxSize / this.count) * (p + 1)) / 2);
            ...
            let pointDist = dist(pointOnCircle.x, pointOnCircle.y, pointOnCircle2.x, pointOnCircle2.y);
            ...
            let circleOffset = (t / PI) * ((this.circleMaxSize / this.count) * (p + 1));
            
            noFill();
            stroke(strokeColor);
            strokeWeight((this.circleMaxSize / this.count)/2);
            drawingContext.setLineDash([radianLength, circumfrence - radianLength]);
            drawingContext.lineDashOffset = (PI * -p) + circleOffset + this.slantedSineWave(256, 0.125, p, 0, 6, p * 0.5);
            
            circle(this.center.x, this.center.y, (this.circleMaxSize / this.count) * (p + 1));
      ...
      slantedSineWave(amplitude, frequency, phase, verticalOffset, speed, slant)
      {
        ...
        return SineWave(amplitude, frequency, phase, verticalOffset, speed, this.time) + slantAddition;
      }
  ...
  class LCDRenderer
    {
      ...
      generateColorArray()
      {
            ...
            for(let x = 0 ; x < this.columnCount; x++)
              {
                ...
                let hueJumpAmount = round(round((255 / this.columnCount) + this.columnCount) * HALF_PI);
                ...
                let brightnessCurrent = map(brightnessCurrentIndex, 0, this.hueDuration - 1, brightnessMinMax.x, brightnessMinMax.y);
                
                let saturationCurrent = 100;
                
                let dashColor = color('hsb(' + hueCurrent + ', ' + saturationCurrent + '%, ' + brightnessCurrent + '%)');
        ...
        let colorTransposeArray = doubleLengthArray[0].map((_, colIndex) => doubleLengthArray.map(row => row[colIndex]));
            ...
            for(let x = 0 ; x < this.columnCount; x++)
              {
                ...
                let rPosition = createVector(rIndex % this.columnCount, floor(rIndex / this.columnCount));
                
                let gIndex = (index + this.greenOffset) % maxIndex;
                let gPosition = createVector(gIndex % this.columnCount, floor(gIndex / this.columnCount));
                
                let bIndex = (index + this.blueOffset) % maxIndex;
                let bPosition = createVector(bIndex % this.columnCount, floor(bIndex / this.columnCount));
                ...
                if(!saturationSwitch)
                  {
                    ...
                    let shiftedBrightness = Clamp(round(brightness(shiftedColor)) - 16, 2, 50);
                    
                    shiftedColor = color('hsb(' + shiftedHue + ', ' + shiftedSaturation + '%, ' + shiftedBrightness + '%)');
                  }
        ...
        for(let x = 0; x < this.columnCount; x++)
          {
            ...
            let randomOffset = floor(random(this.columnOffsetTime / 4, this.columnOffsetTime * 2) * 10) / 10;
            ...
            for(let y = 0 ; y < this.rowCount * 2; y++)
              {
                ...
                let start = createVector((x * columnWidth + (columnWidth / 2)), (y * rowHeight) + (columnWidth - columnGap * 2));
                let end = createVector((x * columnWidth + (columnWidth / 2)), ((y * rowHeight) + (columnWidth - columnGap * 2)) + (rowHeight - rowGap));
                
                // Only offset if true
                if(this.columnOffset)
                  {
                    let yOffset = createVector(0, -(easeInOutQuint(((this.time + (this.columnBaseOffsets[x])) % this.columnOffsetTime) / this.columnOffsetTime) * height));
                start.add(yOffset);
                end.add(yOffset);
                  }
...
function Logo_Topography()
{
  class TopographyRenderer
  {
      ...
      for(let p = 0; p < this.divisions + 4; p++)
        {
          ...
          let start = createVector(-widthMargin, -(heightSpacing / 2) + (heightSpacing * p) - ((heightSpacing / 2) * p));
          let end = createVector(width + widthMargin, -(heightSpacing / 2) + (heightSpacing * p) + ((heightSpacing / 2) * p));
    ...
    renderTopography(time)
    {
      ...
      for(let y = 0; y < this.divisions; y++)
        {
          ...
          let xChange = SineWave(0.41, 0.32, (y + y) * this.changeScale, 0, 1 * this.masterSpeed, this.time);
          let yStartChange = SineWave((height / this.divisions) / 2, 0.082, ((y + y) * y), 0, 5 * this.masterSpeed, this.time);
          let yEndChange = SineWave(-(height / this.divisions) / 2, 0.163, ((y * y) + y), 0, 2 * this.masterSpeed, this.time);
          
          let start = createVector((width * widthChange) + (xChange * width), y * ((height * 1.5) / this.divisions) + yStartChange);
          let end = createVector((width - (width * widthChange)) + (xChange * width), y * ((height * 1.5) / this.divisions) + yEndChange);
      ...
      // Transpose the array
      let transposedArray = pointArray[0].map((_, colIndex) => pointArray.map(row => row[colIndex]));
              ...
              for(let y = 0; y < transposedArray[x].length; y++)
                {
                  ...
                  if(y == transposedArray[x].length / 2)
                    {
                      strokeSize = map(transposedArray[x][y].x, 0, width, SineWave(17, 0.25, -6, 18, 2, this.time), SineWave(17, 0.25, -24, 18, 2, this.time), true);
                    }
                }
    ...
    checkForRenderedPoints(points)
    {
      for(let p = 0; p < points.length; p++)
        {
          if((points[p].x >= 0 && points[p].x <= width) || (points[p].y >= 0 && points[p].y <= height))
            {
              return true;
            }
        }
...
function Logo_Particles()
{
  ...
  {
    constructor(color, emitSize, rate, interval, count, life, shrinkOverLife, random, randomVelocity, smoothness, direction, position, size)
    {
      this.time = 0;
      this.color = color;
    ...
    emitParticle()
    {
      ...
      let emitLocation = createVector(random(-this.size.x/2,this.size.x/2), random(-this.size.y/2,this.size.y/2)).add(emitCenter);
      let emitSize = random(this.emitSize.x, this.emitSize.y);
      let randomVelocity = createVector(random(-this.randomVelocity.x, this.randomVelocity.x), random(-this.randomVelocity.y, this.randomVelocity.y)).add(this.direction);
      
      let particle = new Particle(this, emitLocation, randomVelocity, this.random, this.randomVelocity, emitSize, this.color, this.getNewParticleIndex(),this.smoothness, this.life, this.shrinkOverLife);
  ...
  class Particle
    {
      constructor(emitter, position, velocity, randomWander, randomVelocity, size, color, index, smoothness, life, shrinkOverLife)
      {
      ...
      updateMovement()
      {
        let randomDirection = createVector(random(-this.randomVelocity.x,this.randomVelocity.x), random(-this.randomVelocity.y,this.randomVelocity.y)).mult(this.randomWander);
        let moveDirection = createVector(0,0).add(this.velocity).add(randomDirection);
  ...
  class DotGrid
    {
      ...
      renderGrid(time)
      {
        ...
        for(let x = 0; x < this.col; x++)
          {
            let xOffset = SineWave((this.size.x/this.col), 0.31, x, 0, 1, this.time);
            for(let y = 0; y < this.row; y++)
              {
                let yOffset = SineWave(height * 0.0625, 0.28, y + x, 0, 1.25, this.time);
                let position = createVector(((this.size.x/this.col) * x) + yOffset, ((this.size.y/this.row) * y) + xOffset).add(this.start);
                
                noFill();
                stroke(lerpColor(color('#663ace'), color('#ff75ff'), ((this.col * this.row) - ((this.row * x) + y))* 0.75 / (this.col * this.row)));
                strokeWeight(5);
  ...
  this.setup = function()
  {
    ...
    let emitterD = new ParticleEmitter(color('#ffc862'), createVector(12,16), 2, 0.5, 46, 7, false, 0.5, createVector(16,3), 0.18, createVector(-2, -16), createVector(width/2, height*1.25), createVector(width*1.25, 50));
    let emitterE = new ParticleEmitter(color('#7343ea'), createVector(15,65), 6, 0.3, 81, 4, true, 0.625, createVector(16,18), 0.18, createVector(0, -21), createVector(width/2, height*1.25), createVector(width*1.25, 50));
    let emitterF = new ParticleEmitter(color('#e66cfb'), createVector(12,46), 8, 0.3, 200, 3.125, true, 0.5, createVector(22,18), 0.18, createVector(0, -38), createVector(width/2, height*1.25), createVector(width, 50));
    this.particleEmitters = [emitterA, emitterB, emitterC, emitterD, emitterE, emitterF];
  }
        ...
        if(e == 5)
          {
            noStroke();
            fill(0);
            let gradient = drawingContext.createLinearGradient(0, height*1.25, width, height / 2);
